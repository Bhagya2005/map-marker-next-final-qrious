  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 bg-zinc-900 p-4 rounded-xl border border-white/10 items-center">
        ...
        <button onClick={openCreateModal} className="bg-blue-600 px-4 py-2 rounded-md text-sm font-bold text-white hover:bg-blue-500 transition-colors">
          + Add Pin
        </button>
      </div>
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? (
              <tr><td colSpan={4} className="p-10 text-center animate-pulse text-zinc-500">Loading data...</td></tr>
            ) : pins.map((pin: any) => (
        ...
        <div className="p-4 bg-white/5 border-t border-white/10 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="px-3 py-1 bg-zinc-800 rounded disabled:opacity-50 text-white">Next</button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-[100] backdrop-blur-sm">
          <form onSubmit={handleSave} className="bg-zinc-900 border border-white/10 p-6 rounded-xl w-full max-w-md space-y-4 shadow-2xl">
            ...
            <div className="space-y-3">
               ...
               <div className="flex gap-2">
                 <input type="number" step="any" placeholder="Lat" className="w-full bg-zinc-800 p-2.5 rounded border border-white/10 text-white" value={formData.lat} onChange={e => setFormData({...formData, lat: e.target.value})} required />
                 <input type="number" step="any" placeholder="Lng" className="w-full bg-zinc-800 p-2.5 rounded border border-white/10 text-white" value={formData.lng} onChange={e => setFormData({...formData, lng: e.target.value})} required />
               </div>
            </div>
